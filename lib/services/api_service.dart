...
abstract class ApiService {
  ...
  Future<Map<String, dynamic>> submitSessionData(
    String sessionId,
    CurrentMockSession? payload,
  );
  Future<GenerateQuestionsResponse?> generateQuestionsFromSkills({
    required Analysis analysis,
  });
  Future<GenerateQuestionsResponse?> getSkillSummary(
    String? cvData,
    String? jdData,
  );
  Future<GenerateQuestionsResponse?> getpendingMockInterviewForSession(
    String sessionId,
  );
  ...
  Future<Map<String, dynamic>?> fetchAdaptiveInterviewSummary(
    String sessionId,
  ) async {
    return null;
  }
  ...
  Future<Map<String, dynamic>> getCoachingForSession(
    String sessionId,
    String sessionType,
  );
}
...
class ApiServiceImpl implements ApiService {
  ...
  ApiServiceImpl({required this.authService, required this.baseUrl}) {
    _loadConfig();
  }
  ...
  Future<List<Question>> fetchQuestions(int count) async {
    ...
    for (var i = 0; i < count; i++) {
      final t = _templates.isNotEmpty
          ? _templates[i % _templates.length]
          : null;
      final topic = _topics.isNotEmpty
          ? _topics[i % _topics.length]
          : 'general';
      final text = t != null
      ...
      final id = 'q${i + 1}';
      final tags = t != null
          ? List<String>.from(t['tags'] as List<dynamic>)
          : ['general'];
  ...
  Future<List<Map<String, dynamic>>> fetchCandidateSessions() async {
    ...
    final List<Map<String, dynamic>> data = decoded
  ...
  Future<List<String>> analyseCv(String fileName, String cvText) async {
    ...
    if (resp.statusCode == 200) {
      ...
      final List<dynamic>? rawSkills =
          analysis?['cv_skills']; // can be null or list
  ...
  @override
  Future<GenerateQuestionsResponse?> startSkillBasedSessoin(
    String jsonEncode,
  ) async {
  ...
  Future<List<RoleModel>> fetchRoles() async {
    ...
    final rolesJson = decoded["roles"] as List<dynamic>;
